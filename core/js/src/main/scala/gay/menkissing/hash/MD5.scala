...
object MD5:
  ...
  def littleEndianLong(l: Long): List[Byte] = (0 until 8).map: x =>
    (l >> (x * 8)).toByte
  .toList
  def calc(in: Array[Byte]): Array[Byte] =
    ...
    (0 until (bytes.length / 64)).foreach: j =>
      ...
      (0 until 64).foreach: i =>
        var (f, g) =
          if i <= 15 then ((b & c) | (~b & d), i)
          else if i <= 31 then ((d & b) | (~d & c), 5 * i + 1)
          else if i <= 47 then (b ^ c ^ d, 3 * i + 5)
          else (c ^ (b | ~d), 7 * i)
