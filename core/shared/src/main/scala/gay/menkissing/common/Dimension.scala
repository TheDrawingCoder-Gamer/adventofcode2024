...
final case class Dimension[A] private (min: A, max: A):
  infix def intersect
    ...
    (using ord: Order[A]): Option[Dimension[A]] =
  ...
  infix def -
    ...
    (using ord: Order[A], dis: Discrete[A]): Set[Dimension[A]] =
    this intersect t match
      case Some(h) =>
        var daSet = Set.empty[Dimension[A]]
        if min != h.min then daSet += (min dimBy dis.pred(h.min))
        if max != h.max then daSet += (dis.succ(h.max) dimBy max)
        daSet
      case _ => Set(this)
  ...
  infix def -(n: A)(using ord: Order[A], dis: Discrete[A]): Set[Dimension[A]] =
    if this.contains(n) then
      var daSet = Set.empty[Dimension[A]]
      if min != n then daSet += (min dimBy dis.pred(n))
      if max != n then daSet += (dis.succ(n) dimBy max)
      daSet
    else Set(this)
...
object Dimension:
  ...
  given showDimension[A](using Show[A]): Show[Dimension[A]] =
    dim => show"${dim.min}..${dim.max}"

extension [A](self: A)(using Order[A])
  infix def dimBy(that: A): Dimension[A] = Dimension(self, that)
