...
object Sys3D:
  ...
  enum Direction3D:
    ...
    final def rotate
      ...
      val times2 = times % 4
      // magic
      val gTimes = if times2 < 0 then 4 + times2 else times2
      val ggTimes = (if clockwise then gTimes else 4 - gTimes) % 4
    ...
    given isDirectionN3D: IsDirectionN[Direction3D] with
      ...
      extension (self: Direction3D)
        ...
        def axisDirection: AxisDirection =
          self match
            case Direction3D.West | Direction3D.Down | Direction3D.South =>
  ...
  object AABB3D:
    given showAABB3D[A](using show: Show[A]): Show[AABB3D[A]] =
      box => show"x=${box.xs},y=${box.ys},z=${box.zs}"
