...
object Day14 extends Problem:
    ...
    input.foldLeft(((1L << 36L) - 1L, 0L, Vector.fill(65565)(0L))):
      case ((andM, orM, mem), op) =>
        op match
          case Op.Mask(m) =>
            (
              java.lang.Long.parseLong(m.replace("X", "1"), 2),
              java.lang.Long.parseLong(m.replace("X", "0"), 2),
              mem
            )
  ...
  def part2(input: List[Op]): Long =
    ...
    input.foreach:
      ...
      case Op.Mem(idx, value) =>
        val appliedMask =
          (padLeft(idx.toBinaryString, 36, '0').toList, mask.toList).parMapN:
            case (i, '0') => i
            case (_, m)   => m
