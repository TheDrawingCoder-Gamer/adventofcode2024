...
object Day24 extends ProblemAdv[Day24.Machine, Long, String]:
  ...
  case class Machine(starts: Map[String, Long], eqs: Map[String, BooleanEquation]):
    ...
    def keys(prefix: String): Vector[String] =
      (starts.keySet ++ eqs.keySet).filter(_.startsWith(prefix)).toVector.sorted

    def circuit(key: String, loop: Set[String] = Set.empty): Circuit =
      if loop(key) then InvalidCircuit
      else eqs.get(key) match
        case Some(BooleanEquation(l, r, op)) => op.circuit(circuit(l, loop + key), circuit(r, loop + key))
        case None => CircuitKey(key)

    def inputsTo(key: String): Set[String] = eqs.get(key) match
      case Some(BooleanEquation(l, r, _)) => Set(key) ++ inputsTo(l) ++ inputsTo(r)
      case None => Set.empty
  ...
  enum Op:
    ...
    def circuit(l: Circuit, r: Circuit): Circuit =
      CircuitOp(this, l, r)
  ...
  case class CircuitOp(op: Op, l: Circuit, r: Circuit) extends Circuit:
    override def equals(obj: Any): Boolean =
      obj match
        case CircuitOp(tOp, tL, tR) => (tOp == op) && ((tL == l && tR == r) || (tL == r && tR == l))
        case _ => false

  case class BooleanEquation(l: String, r: String, op: Op)


  def keyOfInt(prefix: String, i: Int) = if i < 10 then s"${prefix}0$i" else s"$prefix$i"
  ...
  def add2bit(i: Int): Circuit =
    ...
    if i == 0 then
      ...
      Op.Xor(Op.Xor(x, y), overflow2bit(i - 1))
  ...
  override def parse(str: String): Machine =
    ...
    val goodStarts = starts.linesIterator.map:
      case s"$x: $y" =>
        x -> y.toLong

    val goodEqs = eqs.linesIterator.map:
      case s"$l $op $r -> $res" => res -> BooleanEquation(l, r, Op.parse(op))

    Machine(goodStarts.toMap, goodEqs.toMap)


  override def part1(input: Machine): Long =
    input.binary("z")
