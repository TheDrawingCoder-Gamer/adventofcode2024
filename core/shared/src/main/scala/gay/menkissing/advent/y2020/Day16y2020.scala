...
object Day16y2020 extends Problem[Day16y2020.DaInput, Long]:

  case class DaInput(validRanges: Map[String, (Range, Range)], ourTicket: List[Int], tickets: List[List[Int]])

  override def parse(str: String): DaInput =
    val Array(ranges, ourTicket, tickets) = str.split("\n\n")
    val rangeMap = ranges.linesIterator.map:
      case s"$k: $r11-$r12 or $r21-$r22" =>
        k -> (r11.toInt to r12.toInt, r21.toInt to r22.toInt)
    .toMap
    val goodTicket = ourTicket.linesIterator.drop(1).next().split(",").map(_.toInt)
    val goodTickets =
  ...
  def part2(input: DaInput): Long =
    ...
    val ordered = goodTickets.prepended(input.ourTicket).transpose.map: xs =>
      input.validRanges.filter:
        case (_, (a, b)) =>
          xs.forall(x => a.contains(x) || b.contains(x))
      .keys.toSet
    ...
    val realOrdered = ordered.zipWithIndex.sortBy(_._1.size).map: (x, i) =>
    ...
    input.ourTicket.zip(realOrdered).filter(_._2.startsWith("departure")).map(_._1.toLong).product
