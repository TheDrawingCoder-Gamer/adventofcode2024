...
object Day12 extends Problem:
  ...
  case class DaState
    (
      x: Int = 0,
      y: Int = 0,
      facing: Direction2D = Direction2D.Right
    ):
  ...
  case class DaStateP2
    ...
    def advance(action: Char, value: Int): DaStateP2 =
      action match
        ...
        case 'L' =>
          copy(waypointRelative = waypointRelative.rotate(-(value / 90)))
  ...
  override def part1(input: List[(Char, Int)]): Int =
    val res =
      input.foldLeft(DaState()):
        case (s, (a, v)) => s.advance(a, v)

    math.abs(res.x) + math.abs(res.y)

  def part2(input: List[(Char, Int)]): Int =
    val res =
      input.foldLeft(DaStateP2()):
        case (s, (a, v)) => s.advance(a, v)
