...
object Day17 extends Problem[List[Int], Int]:
  ...
  def part1(input: List[Int]): Int =
    input.map(it => Box(it)).toSet.subsets().count(_.toList.map(_.value).sum == inputSize)

  def part2(input: List[Int]): Int =
    val values = input.map(it => Box(it)).toSet.subsets().filter(_.toList.map(_.value).sum == inputSize).toList
