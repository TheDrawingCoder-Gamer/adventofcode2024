...
object Day19 extends Problem:
        ...
        case s"$n: $l | $r" =>
          (
            ...
            RawRule.Two(
              ...
              r.split(" ").map(_.toInt).toList
            )
          )
        case s"$n: $x" =>
          (n.toInt, RawRule.One(x.split(" ").map(_.toInt).toList))
      .toMap
  ...
  def getRule(rules: Map[Int, RawRule], k: Int): List[String] =
    ...
    def go(n: Int): List[String] =
      rules(n) match
        ...
        case RawRule.One(x)    => calcResults(x.map(go))
  ...
  def part2(input: (Map[Int, RawRule], List[String])): Int =
    ...
    val r42Parser =
      choice(r42.map(x => Parsley.atomic(parsley.character.string(x)))*)
    val r31Parser =
      choice(r31.map(x => Parsley.atomic(parsley.character.string(x)))*)
    ...
    val r0Parser = (countSome(r42Parser) <~> countSome(r31Parser))
      .filter(_ > _) *> Parsley.eof
