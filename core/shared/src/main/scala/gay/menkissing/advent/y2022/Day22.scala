...
object Day22 extends Problem:
  ...
  extension (s: Seq[GridPos])
    def trimPos: Seq[GridPos] = s.dropWhile(_ == GNil).takeWhile(_ != GNil)
  extension (regionMap: Map[Int, Vec2[Int]])
    def pretty: String =
      val buff = Array.fill(5, 5)('.')
      regionMap.foreach:
        case (region, Vec2(x, y)) => buff(y)(x) = region.toString.head
  ...
  case class ConnectionInfo
    ...
    def traverse(startOffset: Int, maxOffset: Int): Int =
      if flipOffset then maxOffset - 1 - startOffset
      else startOffset
  ...
  object ConnectionParser:
    ...
    def rotated
      ...
      ): (List[String], List[ConnectionInfo]) =
      // Rotating??
      // 0, 0, to 4, 0
      val newNet =
        net.padTo(5, ".....").zipWithIndex
          .flatMap((r, y) => r.zipWithIndex.map((c, x) => c -> (4 - y, x)))
          .sortBy((_, t) => (t._2, t._1)).map(_._1).grouped(5).map(_.mkString)
          .toList.dropWhile(_.forall(_ == '.'))
      val newList =
        list.map:
    ...
    def fullParsed
      (
        net: List[String],
        list: List[ConnectionInfo]
      ): (Map[Int, Vec2[Int]], List[ConnectionInfo]) =
      val regionMap =
        net.zipWithIndex.flatMap: (line, y) =>
    ...
    def parse
      (
        net: List[String],
        list: List[String]
      ): List[(Map[Int, Vec2[Int]], List[ConnectionInfo])] =
      val baseList =
        list.map: str =>
      ...
      val (net4, list4) =
        (
          baseNet.map(_.reverse).transpose.dropWhile(_.forall(it => it == '.'))
            .transpose.map(_.mkString),
          baseList.map(it =>
            it.copy(
              thisDir = it.thisDir.flipHorizontal,
              thatDir = it.thatDir.flipHorizontal
            )
          )
        )
  ...
  case class CompleteGrid
    ...
    val faceWidth =
      math.min(
        grid.rows.map(_.trimPos.size).min,
        grid.columns.map(_.trimPos.size).min
      )
    ...
    def move
      ...
      moveHelper(pos, dir, n): (poses, lastPos) =>
        ...
        val newOffset =
          if flipOffset then faceWidth - 1 - startOffset
          else startOffset
        ...
        val newPos =
          arrivalDir match
            case Direction2D.Up =>
              Vec2(
                (regionPos.x * faceWidth) + newOffset,
                (regionPos.y + 1) * faceWidth - 1
              )
            case Direction2D.Down =>
              Vec2(
                (regionPos.x * faceWidth) + newOffset,
                regionPos.y * faceWidth
              )
            case Direction2D.Left =>
              Vec2(
                (regionPos.x + 1) * faceWidth - 1,
                (regionPos.y * faceWidth) + newOffset
              )
            case Direction2D.Right =>
              Vec2(
                regionPos.x * faceWidth,
                (regionPos.y * faceWidth) + newOffset
              )
        assert(newPos != GNil)
        if grid(newPos) != Open then (lastPos, dir)
        else
          val countUntilEdge =
            poses.indexWhere(it => grid.getOrElse(it, GNil) == GNil)
    ...
    final def moveHelper
      ...
      (
        handleOff: (Seq[Vec2[Int]], Vec2[Int]) => (Vec2[Int], Direction2D)
      ): (Vec2[Int], Direction2D) =
      ...
      if line.forall(it => grid.getOrElse(it, GNil) != Solid) then
        if line.forall(it => grid.getOrElse(it, GNil) != GNil) then
          ...
          handleOff(
            line,
            line.findLast(it => grid.getOrElse(it, GNil) != GNil).get
          )
      else
  ...
  final case class ForbiddenGrid(grid: Grid[GridPos]) extends AnyVal:
    ...
    def completeGrid: CompleteGrid =
      ...
      val (regionMap, conns) =
        netConnections.find: (regionMap, _) =>
    ...
    def move(pos: Vec2[Int], dir: Direction2D, n: Int): Vec2[Int] =
      ...
      moveHelper(pos, dir, n): (line, lastPos) =>
        ...
        val restCount = n - countUntilEdge
        lazy val solidEnd =
          line.findLast(it => grid.getOrElse(it, GNil) != GNil)
      ...
      i match
        ...
        case _  =>
          i.head match
            ...
            case 'L' =>
              helper(i.tail, accum.prepended(Instruction.Counterclockwise))
            case _ =>
