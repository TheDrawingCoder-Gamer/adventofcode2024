...
object Day21 extends Problem[Day21.Actor, Int]:
  ...
  def configurations: List[(Int, Actor)] =
    for
      ...
      rings <- (rings.combinationsN[2].map(_ |+| _) ++ rings).toList.prepended(StoreItem(0, 0, 0))
    yield
  ...
  def part1(boss: Actor): Int =
    ...
    values.filter((cost, a) => a.diesIn(boss.damage) >= boss.diesIn(a.damage)).minBy(_._1)._1
  

  def part2(boss: Actor): Int =
    val values = configurations
    values.filter((cost, a) => a.diesIn(boss.damage) < boss.diesIn(a.damage)).maxBy(_._1)._1