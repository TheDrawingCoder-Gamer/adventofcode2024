...
object Day11y2021 extends Problem[Grid[Int], Long]:
  ...
  def flashElems(grid: Grid[Int]): Grid[Int] = grid.mapWithIndex((l, r) => flashElem(grid, l, r))

  @tailrec
  def flash(grid: Grid[Int]): Grid[Int] =
    val flashed = flashElems(grid)
    if flashed.exists(_ >= 10) then
      flash(flashed)
    else
      flashed
  ...
  def part2(grid: Grid[Int]): Long = simStream(grid).indexWhere(_._1.forall(_ == 0))
