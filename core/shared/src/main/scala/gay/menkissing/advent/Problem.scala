...
extension[Input, Output] (x: WithPart1[Input, Output] & WithInput & ParseP1[Input])
  def fullPart1: Output = x.part1(x.parseP1(x.input))
  def debugAndTimeP1(): Unit =
    val res = debugTiming(fullPart1)
    println(res)

extension[Input, Output](x: WithPart2[Input, Output] & WithInput & ParseP2[Input])
...
trait NewHalfDay[Input, +Output] extends ParseP1[Input], WithPart1[Input, Output], WithInput:
  def main(args: Array[String]): Unit =
    this.debugAndTimeP1()

// HACK HACK HACK!!!!
trait HalfDay[Input, +Output] extends NewHalfDay[Input, Output], WithParser[Input]

trait ProblemSuperAdv[InputP1, InputP2, +OutputP1, +OutputP2] extends NewHalfDay[InputP1, OutputP1], WithPart2[InputP2, OutputP2], ParseP2[InputP2]:
  override def main(args: Array[String]): Unit =
    this.debugAndTimeP1()
    this.debugAndTimeP2()

trait ProblemUniqueInputs[InputP1, InputP2, +Output] extends ProblemSuperAdv[InputP1, InputP2, Output, Output]

trait ProblemAdv[Input, +OutputP1, +OutputP2] extends ProblemSuperAdv[Input, Input, OutputP1, OutputP2], HalfDay[Input, OutputP1]
