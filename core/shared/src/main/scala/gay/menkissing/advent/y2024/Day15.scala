...
object Day15 extends Problem:
  ...
  extension (grid: Grid[GridItem])
    ...
    def updatedMove
      (
        start: Vec2[Int],
        dir: Direction2D
      ): Option[Grid[GridItem]] =
      @tailrec
      def go
        (
          curGrid: Grid[GridItem],
          start: Vec2[Int]
        ): Option[Grid[GridItem]] =
  ...
  extension (grid: Grid[GridItemP2])
    ...
    def updatedMove
      (
        istart: Vec2[Int],
        dir: Direction2D
      ): Option[Grid[GridItemP2]] =
      // ???
      def go
        (
          start: Vec2[Int],
          movedItems: List[(Vec2[Int], GridItemP2)]
        ): Eval[Option[List[(Vec2[Int], GridItemP2)]]] =
        val newPos = start.offset(dir)
        grid(newPos) match
          ...
          case GridItemP2.BoxL  =>
            ...
            dir match
              case Direction2D.Left | Direction2D.Right => go(newPos, newItems)
          ...
          case GridItemP2.BoxR =>
            ...
            dir match
              case Direction2D.Right | Direction2D.Left => go(newPos, newItems)
              case Direction2D.Up | Direction2D.Down    =>
                go(newPos, newItems).flatMap:
  ...
  case class ProblemState
    (
      grid: Grid[GridItem],
      robot: Vec2[Int],
      remainingMoves: List[Direction2D]
    ):
  ...
  override def part1(input: ProblemState): Long =
    ...
    def go(i: ProblemState): ProblemState =
      if i.remainingMoves.nonEmpty then go(i.step.get)
      else i
