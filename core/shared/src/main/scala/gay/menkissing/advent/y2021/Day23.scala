...
object Day23 extends NewProblem:
  ...
  case class State
    ...
    private def roomFree(amphipod: Amphipod): Boolean = (2 to roomSize + 1)
      .flatMap(y => amphipods.get(Vec2(amphipod.room, y))).forall(_ == amphipod)
    ...
    private def validMovesFor(from: Vec2[Int], kind: Amphipod): Seq[Vec2[Int]] =
      ...
      // if amphipod is in the hallway it can go to its destination
        ...
        if roomFree(kind) then
          2.to(roomSize + 1).map(y => Vec2(kind.room, y))
            .findLast(it => !amphipods.contains(it)).toSeq
        else Seq.empty
  ...
  def parseP1(str: String): State = parseShared(str, 2)
  def parseP2(str: String): State =
    val lines = str.linesIterator
    val newInput =
      (lines.take(3) ++ Seq("  #D#C#B#A#", "  #D#B#A#C#") ++ lines.take(2))
        .mkString("\n")
    parseShared(newInput, 4)

  def part1(input: State): Int = search(input).get

  def part2(input: State): Int = search(input).get
