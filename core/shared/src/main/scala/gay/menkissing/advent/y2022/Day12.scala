...
object Day12 extends Problem:
  ...
  case class MountainMap(grid: Grid[Byte]) extends AnyVal:
    ...
    def neighbors(p: Vec2[Int]): Iterable[Byte] =
      neighborVerts(p).map[Byte](p => grid.get(p).getOrElse(28)).toSeq
    // travelling from u to v
    def hasEdge(u: Vec2[Int], v: Vec2[Int]): Boolean =
      if !grid.isDefinedAt(u.x, u.y) || !grid.isDefinedAt(v.x, v.y) then false
      else
        val uv = grid(u)
        val vv = grid(v)
        vv - uv <= 1
    ...
    def filterIndex
      (
        f: (Vec2[Int], Byte) => Boolean
      ): Vector[(Vec2[Int], Byte)] =
  ...
  def part1(input: (Vec2[Int], Vec2[Int], MountainMap)): Int =
    ...
    astar(start, end, _.taxiDistance(end), (_, _) => 1.0, graph.neighborVerts)
      .get.length - 1
