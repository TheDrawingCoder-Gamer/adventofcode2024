  ...
  def parse(str: String): Input =
    str.trim match
      case s"target area: x=$mnx..$mxx, y=$mny..$mxy" =>
        AABB2D(mnx.toInt safeToIncl mxx.toInt, mny.toInt safeToIncl mxy.toInt)

  def willBeInRange(input: AABB2D[Int], initVelocity: Vec2[Int]): Boolean =
    AABB2D(0 safeToIncl input.xs.end, input.ys.start safeToIncl Int.MaxValue)
    var cur = Vec2(0, 0)
    var curV = initVelocity
    // Even if cur.x == xs.max, curV.x could == 0
    while cur.x <= input.xs.end && cur.y > input.ys.start do
      cur += curV
      curV = curV.copy(x = math.max(0, curV.x - 1), y = curV.y - 1)
      if input.contains(cur) then return true
    false

  def xEventuallyEnters(vx: Int, range: Range[Int]): Boolean =
    var curX = 0
    var curV = vx
    while curX < range.end && curV != 0 do
      curX += curV
      curV = math.max(0, curV - 1)
      if range.contains(curX) then return true
    false
  def yEventuallyEnters(vy: Int, range: Range[Int]): Boolean =
    var curY = 0
    var curV = vy
    while curY > range.start do
      curY += curV
      curV -= 1
