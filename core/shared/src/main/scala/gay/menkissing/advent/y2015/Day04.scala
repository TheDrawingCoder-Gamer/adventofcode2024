  ...
  object MD5:
    def calc(in: String, p2: Boolean): Boolean =
      ...
      if p2 then
        ...
        (0 until 2).forall(i => bites(i) == 0) && (bites(2) & 0xf0) == 0

  def part1(input: String): Int =
    Iterator.iterate(1)(_ + 1).find(it => MD5.calc(input + it.toString, false)).get
  def part2(input: String): Int =
    Iterator.iterate(1)(_ + 1).find(it => MD5.calc(input + it.toString, true)).get
