...
object Day10 extends ProblemAdv:
  ...
  def debugPrintInput(input: List[Operation]): String =
    val vec =
      input.zipWithIndex.traverse((l, i) => executeOp(l).map(z => (z, i)))
        .runA(CPU(0, 1)).value.toVector
  ...
  def getCycle(cpus: Vector[CPU])(c: Int): CPU =
    ...
    if firstIndex == -1 then cpus.last
    else cpus.get(firstIndex - 1).getOrElse(CPU(0, 1))

  def getAndNormalize(cpus: Vector[CPU])(c: Int): CPU =
    val cpu = getCycle(cpus)(c)
    cpu.copy(tick = c)
  def signalStrengthPart1(cycle: Int)(cpu: CPU): Int = cycle * cpu.register
  ...
  def processCPU(cpu: CPU): State[CRT, CRT] =
    State: crt =>
      ...
      val good = (cpu.register - 1 to cpu.register + 1)
        .contains(zeroTick % crt.width)
