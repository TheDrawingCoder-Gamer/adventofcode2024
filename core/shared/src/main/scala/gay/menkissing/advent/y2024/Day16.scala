...
object Day16 extends Problem[Day16.ProblemState, Int]:
  ...
  case class Reindeer(pos: Vec2[Int], dir: Direction2D) derives Eq:
    ...
    def edgeScore(that: Reindeer): Double =
      ...
      else if this.dir == that.dir then 1.0
      else if this.dir.clockwise == that.dir ||
        this.dir.counterClockwise == that.dir
      then 1001.0
      else
  ...
  case class ProblemState(reindeer: Reindeer, maze: Maze, end: Vec2[Int]):
    def getSolution: List[Reindeer] =
      astarGeneric[Reindeer](
        reindeer,
        _.pos == end,
        _.pos.taxiDistance(end).toDouble,
        _.edgeScore(_),
        maze.neighbors
      ).get
    ...
    def solve: Int = getSolution.solved
    ...
    def prettySolution(solution: List[Reindeer]): String =
      ...
      maze.values.zipWithIndex.map: (line, y) =>
        line.zipWithIndex.map: (wall, x) =>
          val p = Vec2(x, y)
          if wall then '#'
          else if reindeer.pos == p then 'S'
          else if end == p then 'E'
          else
