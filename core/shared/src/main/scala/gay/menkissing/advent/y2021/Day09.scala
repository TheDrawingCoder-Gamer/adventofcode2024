...
object Day09 extends Problem[Grid[Int], Int]:
  ...
  def followBasin(grid: Grid[Int], p: Vec2[Int], v: Int): (Int, Vec2[Int]) =
    ...
    val (_, lpos) = filteredNs.foldLeft((10, Vec2(-1, -1))):
      case (tv@(v, _), ta@(a, _)) =>
  ...
  def part2(input: Grid[Int]): Int =
    ...
    val basinCounts = basinPos.filter(_ != Vec2(-1, -1)).groupBy(identity).map(_._2.length).toSeq
