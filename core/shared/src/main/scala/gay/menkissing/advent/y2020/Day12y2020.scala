...
object Day12y2020 extends Problem[List[(Char, Int)], Int]:
    ...
    def advance(action: Char, value: Int): DaState =
      action match
        ...
        case 'F' =>
          ...
          val next = Vec2i(x, y) + (facing.digitalDir * value)
  ...
  case class DaStateP2(ship: Vec2i = Vec2i(0, 0), waypointRelative: Vec2i = Vec2i(10, -1)):
    def advance(action: Char, value: Int): DaStateP2 =
      action match {
        case 'N' => copy(waypointRelative = waypointRelative + Vec2i(0, -value))
        case 'S' => copy(waypointRelative = waypointRelative + Vec2i(0, value))
        case 'E' => copy(waypointRelative = waypointRelative + Vec2i(value, 0))
        case 'W' => copy(waypointRelative = waypointRelative + Vec2i(-value, 0))
