...
class ListStatistics(val samples: Vector[Double]):
  def max: Double = samples.maxOption.getOrElse(Double.NaN)
  def min: Double = samples.minOption.getOrElse(Double.NaN)

  def n: Long = samples.size

  def sum: Double = if samples.nonEmpty then samples.sum else Double.NaN

  def mean: Double =
    if n != 0 then sum / n.toDouble
    else Double.NaN
  ...
  def sampleStandardDeviation: Double = math.sqrt(unbiasedVariance)
