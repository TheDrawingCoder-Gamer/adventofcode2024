...
object Gamma {
  ...
  private val LANCZOS = Array(0.99999999999999709182, 57.156235665862923517, -59.597960355475491248, 14.136097974741747174, -0.49191381609762019978, .33994649984811888699e-4, .46523628927048575665e-4, -.98374475304879564677e-4, .15808870322491248884e-3, -.21026444172410488319e-3, .21743961811521264320e-3, -.16431810653676389022e-3, .84418223983852743293e-4, -.26190838401581408670e-4, .36899182659531622704e-5)
  ...
  def logGamma(x: Double): Double = {
    var ret = .0
    if (x.isNaN || (x <= 0.0)) ret = Double.NaN
    else if (x < 0.5) return logGamma1p(x) - math.log(x)
   ...
   */
  def regularizedGammaP(a: Double, x: Double): Double = regularizedGammaP(a, x, DEFAULT_EPSILON, Integer.MAX_VALUE)
   ...
   * @throws MaxCountExceededException if the algorithm fails to converge.
   */
  def regularizedGammaP(a: Double, x: Double, epsilon: Double, maxIterations: Int): Double = {
    var ret = .0
    if (a.isNaN || x.isNaN || (a <= 0.0) || (x < 0.0)) ret = Double.NaN
      ...
      if (n >= maxIterations) throw new Exception(s"Max iteration count exceeded ($maxIterations)")
      else if (sum.isInfinite) ret = 1.0
      else ret = math.exp(-x + (a * math.log(x)) - logGamma(a)) * sum
    }
    ret
  }
   ...
   * @throws MaxCountExceededException if the algorithm fails to converge.
   */
  def regularizedGammaQ(a: Double, x: Double): Double = regularizedGammaQ(a, x, DEFAULT_EPSILON, Integer.MAX_VALUE)
   ...
   * @throws MaxCountExceededException if the algorithm fails to converge.
   */
  def regularizedGammaQ(a: Double, x: Double, epsilon: Double, maxIterations: Int): Double = {
    var ret = .0
    if (a.isNaN || x.isNaN || (a <= 0.0) || (x < 0.0)) ret = Double.NaN
      ...
      val cf: ContinuedFraction = new ContinuedFraction() {
        /** {@inheritDoc } */
        override def getA(n: Int, x: Double): Double = {
          ((2.0 * n) + 1.0) - a + x
        }
  ...
  def invGamma1pm1(x: Double): Double = {
    if (x < -0.5) throw new Exception(s"$x is smaller than 0.5")
    if (x > 1.5) throw new Exception(s"$x is greater than 1.5")
    var ret = .0
    val t = if (x <= 0.5) x
    else (x - 0.5) - 0.5
    if (t < 0.0) {
  ...
  def logGamma1p(x: Double): Double = {
    if (x < -0.5) throw new Exception(s"Number $x is too small (smaller than -0.5)")
    if (x > 1.5) throw new Exception(s"Number $x is too large (larger than 1.5)")
  ...
  def gamma(x: Double): Double = {
    if ((x == math.rint(x)) && (x <= 0.0)) return Double.NaN
    var ret = .0
    val absX = math.abs(x)
    if (absX <= 20.0) if (x >= 1.0) {
      /*
      ...
      val gammaAbs = SQRT_TWO_PI / absX * math.pow(y, absX + 0.5) * math.exp(-y) * lanczos(absX)
      if (x > 0.0) ret = gammaAbs
      else {
        /*
