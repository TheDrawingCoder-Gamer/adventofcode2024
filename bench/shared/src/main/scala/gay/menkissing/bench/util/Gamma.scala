...
object Gamma:
   ...
   */
  def regularizedGammaP(a: Double, x: Double): Double =
   ...
   * Regularized Gamma Function</a>, equation (1) </li> <li> <a
   * href="http://mathworld.wolfram.com/IncompleteGammaFunction.html">
   * Incomplete Gamma Function</a>, equation (4). </li> <li> <a
  ...
  def regularizedGammaP
    (a: Double, x: Double, epsilon: Double, maxIterations: Int): Double =
    var ret = .0
    if a.isNaN || x.isNaN || (a <= 0.0) || (x < 0.0) then ret = Double.NaN
    else if x == 0.0 then ret = 0.0
    else if x >= a + 1 then
      ...
      if n >= maxIterations then
        throw new Exception(s"Max iteration count exceeded ($maxIterations)")
      else if sum.isInfinite then ret = 1.0
   ...
   *   if the algorithm fails to converge.
   */
  def regularizedGammaQ(a: Double, x: Double): Double =
   ...
   *   if the algorithm fails to converge.
   */
  def regularizedGammaQ
    (a: Double, x: Double, epsilon: Double, maxIterations: Int): Double =
    var ret = .0
    if a.isNaN || x.isNaN || (a <= 0.0) || (x < 0.0) then ret = Double.NaN
    else if x == 0.0 then ret = 1.0
      ...
      val cf: ContinuedFraction =
        new ContinuedFraction():
          ...
          /** {@inheritDoc } */
          override def getB(n: Int, x: Double): Double = n * (a - n)
  ...
  def digamma(x: Double): Double =
    if x.isNaN || x.isInfinite then return x
    if x > 0 && x <= S_LIMIT then
  ...
  /**
   * Computes the trigamma function of x. This function is derived by taking the
   * derivative of the implementation of digamma.
   *
   * @param x
   *   Argument.
   * @return
   *   trigamma(x) to within 10-8 relative or absolute error whichever is
   *   smaller
   * @see
  ...
  def trigamma(x: Double): Double =
    if x.isNaN || x.isInfinite then return x
    if x > 0 && x <= S_LIMIT then return 1 / (x * x)
    if x >= C_LIMIT then
      val inv = 1 / (x * x)
      //  1    1      1       1       1
      //  - + ---- + ---- - ----- + -----
