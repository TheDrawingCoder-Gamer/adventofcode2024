...
object Beta {
   ...
   */
  def regularizedBeta(x: Double, a: Double, b: Double): Double = regularizedBeta(x, a, b, DEFAULT_EPSILON, Integer.MAX_VALUE)
   ...
   * if the algorithm fails to converge.
   */
  def regularizedBeta(x: Double, a: Double, b: Double, epsilon: Double): Double = regularizedBeta(x, a, b, epsilon, Integer.MAX_VALUE)
   ...
   * if the algorithm fails to converge.
   */
  def regularizedBeta(x: Double, a: Double, b: Double, maxIterations: Int): Double = regularizedBeta(x, a, b, DEFAULT_EPSILON, maxIterations)
   ...
   * if the algorithm fails to converge.
   */
  def regularizedBeta(x: Double, a: Double, b: Double, epsilon: Double, maxIterations: Int): Double = {
    var ret = .0
    if (x.isNaN || a.isNaN || b.isNaN || x < 0 || x > 1 || a <= 0 || b <= 0) ret = Double.NaN
    else if (x > (a + 1) / (2 + b + a) && 1 - x <= (b + 1) / (2 + b + a)) ret = 1 - regularizedBeta(1 - x, b, a, epsilon, maxIterations)
    else {
      ...
      ret = math
        .exp((a * math.log(x)) + (b * math.log1p(-x)) - math.log(a) - logBeta(a, b)) * 1.0 / fraction
   ...
   */
  @deprecated def logBeta(a: Double, b: Double, epsilon: Double, maxIterations: Int): Double = logBeta(a, b)
  ...
  private def logGammaSum(a: Double, b: Double) = {
    if ((a < 1.0) || (a > 2.0)) throw new Exception(s"$a is out of range 1.0, 2.0")
    if ((b < 1.0) || (b > 2.0)) throw new Exception(s"$b is out of range 1.0, 2.0")
    val x = (a - 1.0) + (b - 1.0)
    if (x <= 0.5) Gamma.logGamma1p(1.0 + x)
    else if (x <= 1.5) Gamma.logGamma1p(x) + math.log1p(x)
  ...
  private def logGammaMinusLogGammaSum(a: Double, b: Double) = {
    if (a < 0.0) throw new Exception(s"$a is too small (smaller than 0.0)")
    if (b < 10.0) throw new Exception(s"$b is too small (smaller than 10.0)")
    /*
             * d = a + b - 0.5
             */
    var d = .0
  ...
  private def deltaMinusDeltaSum(a: Double, b: Double) = {
    if ((a < 0) || (a > b)) throw new Exception(s"$a is out of range 0 to $b")
    if (b < 10) throw new Exception(s"$b is too small (less than 10.0)")
    val h = a / b
    val p = h / (1.0 + h)
    val q = 1.0 / (1.0 + h)
    val q2 = q * q
    ...
    for (i <- DELTA.length - 2 to 0 by -1) {
      w = t * w + DELTA(i) * s(i)
    }
    w * p / b
  }
  ...
  private def sumDeltaMinusDeltaSum(p: Double, q: Double) = {
    if (p < 10.0) throw new Exception(s"$p is too small (< 10)")
    if (q < 10.0) throw new Exception(s"$q is too small (< 10)")
    val a = math.min(p, q)
    val b = math.max(p, q)
    val sqrtT = 10.0 / a
    val t = sqrtT * sqrtT
  ...
  def logBeta(p: Double, q: Double): Double = {
    if (p.isNaN || q.isNaN || (p <= 0.0) || (q <= 0.0)) return Double.NaN
    val a = math.min(p, q)
    val b = math.max(p, q)
    if (a >= 10.0) {
      ...
      if (u <= v) (((-0.5 * math.log(b) + HALF_LOG_TWO_PI) + w) - u) - v
      else (((-0.5 * math.log(b) + HALF_LOG_TWO_PI) + w) - v) - u
    }
    else if (a > 2.0) if (b > 1000.0) {
      ...
      (math.log(prod) - n * math.log(b)) + (Gamma.logGamma(ared) + logGammaMinusLogGammaSum(ared, b))
    }
    else {
      var prod1 = 1.0
      var ared = a
      ...
      if (b < 10.0) {
        ...
        math.log(prod1) + math.log(prod2) + (Gamma.logGamma(ared) + (Gamma
          .logGamma(bred) - logGammaSum(ared, bred)))
      }
      else math.log(prod1) + Gamma.logGamma(ared) + logGammaMinusLogGammaSum(ared, b)
    }
    else if (a >= 1.0) if (b > 2.0) if (b < 10.0) {
      var prod = 1.0
      var bred = b
      while (bred > 2.0) {
        bred -= 1.0
      ...
      math.log(prod) + (Gamma.logGamma(a) + (Gamma.logGamma(bred) - logGammaSum(a, bred)))
    }
    else Gamma.logGamma(a) + logGammaMinusLogGammaSum(a, b)
    else Gamma.logGamma(a) + Gamma.logGamma(b) - logGammaSum(a, b)
    else if (b >= 10.0) Gamma.logGamma(a) + logGammaMinusLogGammaSum(a, b)
    else {
